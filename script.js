...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Détecte l'appareil → charge la bonne source → fade-in au canplay
    if (video) {
        ...
        // Si la vidéo est déjà en cache → readyState >= 3, canplay a déjà eu lieu
        if (video.readyState >= 3) {
            showVideo();
        } else {
            // canplay = premiers octets reçus, loadeddata = fallback
            video.addEventListener('canplay',    showVideo, { once: true });
            video.addEventListener('loadeddata', showVideo, { once: true });
            // Fallback ultime : 3s (connexion très lente ou bug navigateur)
