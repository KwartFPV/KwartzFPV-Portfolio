...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ░░ 5. SMART VIDEO LOADER ░░
    if (video) {
        const isMobile = window.innerWidth <= 900 || ('ontouchstart' in window && window.innerWidth <= 1024);
        const src = isMobile ? 'assets/header-video-osd.webm' : 'assets/header-video.webm';

        // Guard : showVideo ne s'exécute qu'une seule fois
        let shown = false;
        const showVideo = () => {
            if (shown) return;
            shown = true;
        ...
        ['canplay', 'canplaythrough', 'loadeddata', 'playing'].forEach(evt => {
            video.addEventListener(evt, showVideo, { once: true });
        });

        // Fallback garanti : 1.5s max — couvre cache, autoplay bloqué, etc.
        setTimeout(showVideo, 1500);

        video.load();

        // Reprise si onglet redevient visible
